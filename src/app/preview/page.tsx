...
export default function HomepagePreview(){
  ...
  return <div className="bg-white text-zinc-900">
    ...
    {categoryModules.map(category=><section id={category.slug} key={category.slug} className="scroll-mt-24 border-t border-violet-100 bg-white">
      <header className="relative min-h-[330px] overflow-hidden bg-[#333132] text-white md:min-h-0" style={{aspectRatio:'1920 / 330'}}><img src={`/preview-assets/categories-html/${category.slug}-header.webp`} alt="" className="absolute inset-0 h-full w-full object-cover object-center" /><div className="absolute inset-0 bg-black/5" /><div className="relative mx-auto flex h-full min-h-[330px] max-w-[1500px] items-center px-5 py-10 sm:px-8 md:min-h-0"><div className="w-full text-center md:ml-[24%] md:w-[55%] md:text-left"><h2 className="text-3xl font-semibold leading-tight tracking-tight sm:text-4xl lg:text-5xl">{category.title}</h2><p className="mt-4 max-w-3xl text-sm leading-6 text-white/70 sm:text-base sm:leading-7">{category.sellingPoint}</p><div className="mt-5 flex flex-wrap justify-center gap-2 md:justify-start">{category.features.map(feature=><span key={feature} className="rounded-full border border-violet-200/25 bg-violet-200/10 px-3 py-1.5 text-xs font-semibold text-violet-100 backdrop-blur-sm">{feature}</span>)}</div></div></div></header>
      <div className="relative overflow-hidden bg-[#faf9ff]"><img src={`/preview-assets/categories-html/${category.slug}-featured.webp`} alt={`${category.title} featured product and usage scene`} className="block h-auto w-full object-contain" /><div className="absolute left-[7%] top-[12%] rounded-full border border-violet-200 bg-white/90 px-4 py-2 text-xs font-black uppercase tracking-[0.14em] text-violet-800 shadow-sm backdrop-blur-sm">Featured Product</div><div className="relative flex flex-col gap-3 px-5 pb-8 pt-5 sm:flex-row sm:justify-center md:absolute md:bottom-[14%] md:right-[7%] md:p-0"><a href={category.href} target="_blank" rel="noreferrer" className="inline-flex min-h-11 items-center justify-center rounded-full bg-violet-700 px-6 py-3 text-sm font-bold text-white transition hover:-translate-y-0.5 hover:bg-violet-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500 focus-visible:ring-offset-2">{category.viewLabel}</a><button type="button" onClick={()=>requestCategoryQuote(category.title)} className="inline-flex min-h-11 items-center justify-center rounded-full border-2 border-violet-700 bg-white/90 px-6 py-3 text-sm font-bold text-violet-800 transition hover:-translate-y-0.5 hover:bg-violet-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500 focus-visible:ring-offset-2">Request a Quote</button></div></div>
      <div className="overflow-x-auto bg-[#faf9ff] px-4 pb-12"><img src={`/preview-assets/categories-html/${category.slug}-products.webp`} alt={`${category.title} product range`} className="mx-auto h-auto min-w-[760px] max-w-[1200px] object-contain" /></div>
    </section>)}
