...
export default function HomepagePreview(){
  ...
  return <div className="bg-white text-zinc-900">
    ...
    {categoryModules.map(category=><section id={category.slug} key={category.slug} className="scroll-mt-24 border-t border-violet-100 bg-white">
      <header className="relative min-h-[330px] overflow-hidden bg-[#333132] text-white md:min-h-0" style={{aspectRatio:'1920 / 330'}}><img src={`/preview-assets/categories-html/${category.slug}-header.webp`} alt="" className="absolute inset-0 h-full w-full object-cover object-center" /><div className="absolute inset-0 bg-black/5" /><div className="relative mx-auto flex h-full min-h-[330px] max-w-[1500px] items-center px-5 py-10 sm:px-8 md:min-h-0"><div className="w-full rounded-2xl bg-[#333132]/75 p-5 text-center backdrop-blur-sm md:ml-[24%] md:w-[58%] md:bg-transparent md:p-0 md:text-left md:backdrop-blur-none"><h2 className="text-3xl font-semibold leading-tight tracking-tight [text-shadow:0_2px_12px_rgba(0,0,0,.45)] sm:text-4xl lg:text-5xl">{category.title}</h2><p className="mt-4 max-w-3xl text-sm leading-6 text-white/80 [text-shadow:0_1px_8px_rgba(0,0,0,.55)] sm:text-base sm:leading-7">{category.sellingPoint}</p><div className="mt-5 flex flex-wrap justify-center gap-2 md:justify-start">{category.features.map(feature=><span key={feature} className="rounded-full border border-violet-200/30 bg-[#3a294a]/45 px-3 py-1.5 text-xs font-semibold text-violet-50 backdrop-blur-sm">{feature}</span>)}</div></div></div></header>
      <div className="relative overflow-hidden bg-[#faf9ff]"><a href={category.featuredHref} target="_blank" rel="noreferrer" aria-label={`View featured ${category.title}`} className="block focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-violet-500"><img src={`/preview-assets/categories-html/${category.slug}-featured.webp`} alt={`${category.title} featured product and usage scene`} className="block h-auto w-full object-contain" /></a><div className="absolute left-[7%] top-[12%] rounded-full border border-violet-200 bg-white/90 px-4 py-2 text-xs font-black uppercase tracking-[0.14em] text-violet-800 shadow-sm backdrop-blur-sm">Featured Product</div><a href={category.featuredHref} target="_blank" rel="noreferrer" className="absolute bottom-[14%] left-[7%] rounded-full bg-white/95 px-5 py-2.5 text-base font-black text-violet-900 shadow-md transition hover:bg-violet-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500">{category.featuredPrice}</a><div className="relative flex flex-col gap-3 px-5 pb-8 pt-5 sm:flex-row sm:justify-center md:absolute md:bottom-[14%] md:right-[7%] md:p-0"><a href={category.href} target="_blank" rel="noreferrer" className="inline-flex min-h-11 items-center justify-center rounded-full bg-violet-700 px-6 py-3 text-sm font-bold text-white transition hover:-translate-y-0.5 hover:bg-violet-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500 focus-visible:ring-offset-2">{category.viewLabel}</a><button type="button" onClick={()=>requestCategoryQuote(category.title)} className="inline-flex min-h-11 items-center justify-center rounded-full border-2 border-violet-700 bg-white/90 px-6 py-3 text-sm font-bold text-violet-800 transition hover:-translate-y-0.5 hover:bg-violet-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500 focus-visible:ring-offset-2">Request a Quote</button></div></div>
      <div className="overflow-x-auto bg-[#faf9ff] px-4 pb-12"><div className="mx-auto min-w-[760px] max-w-[1200px]"><div className="relative"><img src={`/preview-assets/categories-html/${category.slug}-products.webp`} alt={`${category.title} product range`} className="block h-auto w-full object-contain" /><div className="absolute inset-0 grid grid-cols-4">{category.products.map((product,index)=><a key={`image-${product.href}-${index}`} href={product.href} target="_blank" rel="noreferrer" title={`View ${category.title} product ${index+1}`} aria-label={`View ${category.title} product ${index+1}, ${product.price}`} className="border-r border-transparent transition hover:bg-violet-500/5 hover:ring-2 hover:ring-inset hover:ring-violet-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-violet-500" />)}</div></div><div className="grid grid-cols-4 border-x border-b border-violet-100 bg-white">{category.products.map((product,index)=><a key={`price-${product.href}-${index}`} href={product.href} target="_blank" rel="noreferrer" aria-label={`View ${category.title} product ${index+1}, ${product.price}`} className="flex min-h-20 flex-col items-center justify-center border-r border-violet-100 px-2 py-3 text-center last:border-r-0 hover:bg-violet-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-violet-500"><span className="text-base font-black text-[#3d2758]">{product.price}</span><span className="mt-1 text-xs font-semibold text-violet-700">View Product →</span></a>)}</div></div></div>
    </section>)}
