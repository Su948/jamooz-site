...
export default function HomepagePreview(){
  ...
  return <div className="bg-white text-zinc-900">
    ...
    {categoryModules.map(category=><section id={category.slug} key={category.slug} className="scroll-mt-24 border-t border-violet-100 bg-white">
      <header className="relative min-h-[330px] overflow-hidden bg-[#333132] text-white md:min-h-0" style={{aspectRatio:'1920 / 330'}}><img src={`/preview-assets/categories-html/${category.slug}-header.webp`} alt="" className="absolute inset-0 h-full w-full object-cover object-center" /><div className="absolute inset-0 bg-black/10" /><div className="relative mx-auto flex h-full min-h-[330px] max-w-[1500px] items-center px-5 py-10 sm:px-8 md:min-h-0"><div className="w-full rounded-2xl bg-[#333132]/80 p-5 text-center shadow-xl backdrop-blur-md md:ml-[24%] md:w-[58%] md:p-7 md:text-left"><h2 className="text-3xl font-semibold leading-tight tracking-tight sm:text-4xl lg:text-5xl">{category.title}</h2><p className="mt-4 max-w-3xl text-sm leading-6 text-white/75 sm:text-base sm:leading-7">{category.sellingPoint}</p><div className="mt-5 flex flex-wrap justify-center gap-2 md:justify-start">{category.features.map(feature=><span key={feature} className="rounded-full border border-violet-200/25 bg-violet-200/10 px-3 py-1.5 text-xs font-semibold text-violet-100 backdrop-blur-sm">{feature}</span>)}</div></div></div></header>
